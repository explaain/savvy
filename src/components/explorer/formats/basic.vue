<template lang="html">
  <div class="content">
    ...
    <list v-if="content.listItems || content.cells" :listItems="content.listItems || {value: content.cells.value.map((cell, i) => i), state: 'pending'}" :listCards="content.listCards || {value: content.cells.value.map(cell => { return { description: cell.content } }), state: 'pending'}" :full="full" :allCards="allCards" :editing="editing"></list>
